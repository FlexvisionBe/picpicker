...
function uncheck_safe(v) {
    ...
    $("input.uncheck-safe[name=" + name_wo_special + "]").remove();
    ...
    if ($(v).is(":checked")) {
        ...
        /*debug: $('<input class="uncheck-safe" type="text" name="' + name + '" value="' + name + '">').insertAfter($(v).parents("label"));*/
...
function insert_from_exterior(v){
	...
	if(last_group_target.is(".manual_increment")){
		...
		element.find("[data-lang-iso]").each(function(){
			...
			if(element.find("[data-inner_child]").length>0){
				element.find("[data-inner_child]").find("[data-name]").each(function(){
					var keyword_innerchild = $(this).closest("[data-inner_child]").attr("data-inner_child");
					$(this).attr("name",structure_width_new_level+"["+keyword_innerchild+"]["+lang_iso+"]"+"[0]"+"["+$(this).attr("data-name")+"]");
				});
			}
		});
			...
			element.find("[data-form-base]").each(function(){
				if($(this).find("[data-name]").length >= 1){
					...
					$(this).find("input, textarea, select").each(function(){
						$(this).attr("name",structure_width_new_level)
					});
				}
			});
		}
